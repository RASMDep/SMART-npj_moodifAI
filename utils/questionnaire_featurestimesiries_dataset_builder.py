...
def process_participant(data_dir, questionnaire_answers, ids_p, out_dir):
    dataset = {}
    ...
    for pid in tqdm.tqdm(questionnaire_answers['key_smart_id'].unique()):
        ...
        try:
            ...
            for _, row in quest.iterrows():
                ...
                end_time = pd.to_datetime(quest_time_utc, unit='s')

                filtered_hrv = load_and_resample(data_dir, pid, 'hrvmetrics', f"{pid}_hrvmetrics_winlen5_overlap_0.csv", '5T', start_time_24_hours_before, end_time)
                if nan_percentage(filtered_hrv['HRV_MeanNN']) > 30:
                    continue

                filtered_acc = load_and_resample(data_dir, pid, 'accmetrics', f"{pid}_accmetrics_winlen_5.csv", '5T', start_time_24_hours_before, end_time)
...
process_participant(data_dir, questionnaire_answers, ids_p, out_dir)

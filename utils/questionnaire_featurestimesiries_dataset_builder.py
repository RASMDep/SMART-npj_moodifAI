...
for pid in questionnaire_answers.key_smart_id.unique():
    try:
        ...
        for _, row in tqdm.tqdm(quest.iterrows()):
            ...
            filtered_acc = resampled_hrv[(resampled_acc.index >= start_time_24_hours_before) & (resampled_acc.index  < pd.to_datetime(start_time, unit='s'))]
            filtered_acc = resampled_acc.reset_index()
            ...
            if filtered_hrv is not None and filtered_acc is not None:
                ...
                with open(os.path.join(out_dir, "HRV_ACC_timeseries_24hours.pkl"), 'wb') as file:
